...
#pragma warning disable RS2008 // enable analyzer release tracking
	...
	public static readonly DiagnosticDescriptor NonStaticHookMethod = new(
		...
		messageFormat:
		"Hook method '{0}' should be a plain static method; instance methods / capturing lambdas can cause all sorts of chaos by capturing state, and Action/Func/etc objects make it hard to pinpoint what actually gets used as the hook",
		category: "Usage",
		defaultSeverity: DiagnosticSeverity.Warning,
		isEnabledByDefault: true
	);

	public static readonly DiagnosticDescriptor GenericHookMethod = new(
		id: "RM0206",
		title: "Hook methods should be non-generic",
		messageFormat:
		"Hook method '{0}' should be non-generic; a limitation of MonoMod is that, for generics, you must hook a specific closed generic instantiation such as SomeMethod<int> instead of the open SomeMethod<T>",
		category: "Usage",
		defaultSeverity: DiagnosticSeverity.Warning,
		isEnabledByDefault: true
	);
	...
	public static readonly DiagnosticDescriptor NonNullableHookParameter = new(
		id: "RM0212",
		title: "Reference type hook parameters should be nullable",
		messageFormat:
		"Hook method parameter '{0}' should be marked nullable; Celeste/Everest aren't nullable-aware, so reference type parameters other than `orig` and `self` should be maybe-null to catch potential null reference bugs",
		category: "Usage",
		defaultSeverity: DiagnosticSeverity.Warning,
		isEnabledByDefault: true
	);
	...
	public static readonly DiagnosticDescriptor OrigNotCalled = new(
		...
		messageFormat:
		"'{0}' should call its orig(...) delegate in at least one code path to give the rest of the detour chain a chance to run; consider making your changes conditional, e.g only if some setting is active or the player is in some map",
		category: "Usage",
		defaultSeverity: DiagnosticSeverity.Warning,
		isEnabledByDefault: true
	);
	...
	public static readonly DiagnosticDescriptor YieldReturnOrig = new(
		...
		messageFormat:
		"Due to how Celeste's coroutine system works, `yield return orig(...)` introduces a 1-frame delay; consider using `yield return new SwapImmediately(orig(...))` instead",
		category: "Usage",
		defaultSeverity: DiagnosticSeverity.Warning,
		isEnabledByDefault: true
	);
	...
	public static readonly DiagnosticDescriptor DontUseSceneAsMethod = new(
		...
		messageFormat:
		"Instead of `SceneAs<T>`, use `((T)Scene)` if a type mismatch should throw or `(Scene as T)` if you expect the cast to possibly fail; it's functionally identical, more idiomatic, and avoids erasing the nullable annotation since Celeste/Monocle aren't nullable-aware",
		category: "Usage",
		defaultSeverity: DiagnosticSeverity.Warning,
		isEnabledByDefault: true
	);

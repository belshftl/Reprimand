...
- probably scrap the current `[OnLoad]`/`[OnUnload]` split and instead make `[OnLoad]` take in an "undo method" parameter
- probably also dedicated attributes for declaring hooks since they can provide extra niceties like autogenerated per-hook detour IDs
